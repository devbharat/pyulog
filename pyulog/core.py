...
class ULog(object):
    ...
    class MessageLoggingObc(object):
        """ ULog logged string message from Onboard Computer representation """
    ...
    def _read_file_header(self):
        ...
        # dump crc
        crc = self._file_handle.read(2)
        calc_crc = crc16xmodem(header_data, crc16xmodem([0xA1]))
        if (calc_crc != (crc[1] << 8 | crc[0])):
            raise Exception("CRC match failed FILE_HEADER")
    ...
    def _read_file_definitions(self):
        ...
        while True:
            ...
            header_crc = crc16xmodem(data, crc16xmodem([0xA1]))
            print("run count", run_cnt)
            run_cnt = run_cnt + 1
            data = self._file_handle.read(header.msg_size)
            crc = self._file_handle.read(2) # crc
            calc_crc = crc16xmodem(data, header_crc)
            if (calc_crc != (crc[1] << 8 | crc[0])):
                print(data, crc, calc_crc)
                raise Exception("CRC match failed FILE_DEFINITION")
            ...
            if header.msg_type == self.MSG_TYPE_INFO:
                ...
                if self._file_handle.tell() != 16 + 2 + 3 + header.msg_size + 2: # crc for header and flag bits msg
